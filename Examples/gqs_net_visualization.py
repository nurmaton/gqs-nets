...
import math, itertools, sys, json, re, textwrap, os
import numpy as np
...
def _main():
    ...
    INFO = ("HOW THE CHECKS ARE COMPUTED (all from the current vertex\n"
            ...
            "  lists the parameter of each frame).\n"
    ...
    reg('col', b_svg.ax, b_png.ax, b_obj.ax, b_seq.ax)
    ...
    b_svg.on_clicked(save_svg); b_png.on_clicked(save_png); b_obj.on_clicked(save_obj); b_seq.on_clicked(save_motion)
    fig._widgets = state['widgets'] = [sl, b_minus, b_plus, b_vert, b_face, b_edge, b_angl, b_hid, b_shad, b_rep, b_reset, b_dih, b_frz, b_par, b_col, *b_light.values(), *cdl['buttons'],
                                       b_svg, b_png, b_obj, b_seq, b_info, b_x_rep, b_x_face]   # keep the widgets alive
