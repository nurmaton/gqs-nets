...
RENDER_SAMPLES, RENDER_ENGINE, RENDER_SIZE = 128, "cycles", "1920x1080"   # options passed to render_net.py
...
def _main():
    ...
    INFO = ("HOW THE CHECKS ARE COMPUTED (all from the current vertex\n"
            ...
            "  with the built-in Blender recipe (Blender is found automatically, or set BLENDER_PATH;\n"
            "  a render_net.py next to this script replaces the built-in recipe).\n"
    ...
    b_svg = FancyButton(fig, [xs_[0], 0.088, wts[0]*unit_, H], 'SVG', fontsize=8.5); b_png = FancyButton(fig, [xs_[1], 0.088, wts[1]*unit_, H], 'PNG', fontsize=8.5)
    b_obj = FancyButton(fig, [xs_[2], 0.088, wts[2]*unit_, H], 'OBJ', fontsize=8.5); b_seq = FancyButton(fig, [xs_[3], 0.088, wts[3]*unit_, H], 'Motion', fontsize=8.5)
    b_rend = FancyButton(fig, [X0, 0.050, XW, H], 'Render in Blender', fontsize=8.5)
    ...
    if "--frames" in sys.argv:          # headless: save a few frames and exit (optional: --view ELEV,AZIM)
        ...
        for k, t in enumerate(np.linspace(lo, hi, 6)):
            ...
            try:
                sl.eventson = False; sl.set_val(float(t)); sl.eventson = True; b_info.set_on(k == 1)   # the slider and the (i) button follow the frame
...
if key_dir.dot(w) < 0:                                              # the key behind the visible side (the world sun seen from
...
def _frame_pts():
    for vtx in net.data.vertices:
        ...
        if not below and shadow_dir.z > 0.05:
...
if below:
    ...
    d_sh = 6.0*size
    shadow_light = area_light("shadow", cen + d_sh*shadow_dir, (1.0, 1.0, 1.0), 0.5*E_tot*math.pi*d_sh**2, 1.5*size)   # soft, compact footprint
